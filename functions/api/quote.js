import { quoteAdminEmail, quoteCustomerEmail } from './_email-templates.js';
...
export async function onRequestPost(context) {
  try {
    ...
    const { naam, telefoon, email, adres, diensten, bericht } = body;

    if (!naam || !telefoon || !email || !adres || !diensten) {
    ...
    await sendEmail({
      ...
      bcc: [BCC_EMAIL],
      replyTo: email,
      subject: `Nieuwe offerte aanvraag van ${naam} — ${diensten}`,
      html: quoteAdminEmail({ naam, telefoon, email, adres, diensten, bericht }),
    });
    ...
    await sendEmail({
      from: FROM_EMAIL,
      to: [email],
      subject: 'Uw offerte aanvraag is ontvangen — Blink Schoonmaak 010',
      html: quoteCustomerEmail({ naam, diensten, adres }),
    });
